...
YouTube channel links (`/@handle`, `/channel/...`, `/c/...`, and `/user/...`) also offer a **YouTube Channel Catalogue** preset. Paste one or several channel links into the main input; they are grouped into one result and one **Export all channels** action that creates a combined `youtube_export/youtube_videos.json` and `youtube_export/youtube_videos.xlsx` catalogue. The export reads the standard Videos tab only and checkpoints completed video metadata so interrupted runs can resume.
